...
function annotatorImageSelect(options) {
  ...
  var imgselect_utils = {
    ...
    drawImageHighlight: function(annotation) {
      ...
      var img = $('canvas[id$="' + imgselection.src + '"], svg[id$="' + imgselection.src + '"], img[id$="' + imgselection.src + '"], div[id$="' + imgselection.src + '"]').first();
      if (img.length === 0) {
    ...
    imageSelection: function(img, selection) {
      ...
      var w = (selection.x2 - selection.x1) / imgWidth,
         h = (selection.y2 - selection.y1) / imgHeight;
      return {
        // full uri to the image
        uri: img.id,
        // store src as it appears in the document, so we can find it again
        src: $(img).attr('id'),
        x: percent(selection.x1 / imgWidth),
        y: percent(selection.y1 / imgHeight),
        w: percent(w),
        h: percent(h)
      };
    },
  ...
  return {
    ...
    // when annotator starts, load & configure image selection
      ...
      beforeAnnotationDeleted: function(annotation) {
        ...
        if (annotation.id != undefined && annotation.image_selection) {
